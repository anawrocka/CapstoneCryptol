...
%token
  ...
  'spec'      { Located $$ (Token (KW KW_spec     ) _)}
  'then'      { Located $$ (Token (KW KW_then   ) _)}
  'when'      { Located $$ (Token (KW KW_when   ) _)}
  ...
  '\x2295'         { Located $$ (Token (Op Xor         ) _)}
  '\x2228'        { Located $$ (Token (Op Disj        ) _)}
  '\x2227'        { Located $$ (Token (Op Conj        ) _)}
  ...
  '\x2264'        { Located $$ (Token (Op LEQ         ) _)}
  '\x2265'        { Located $$ (Token (Op GEQ         ) _)}
  ...
  '\x00AC'         { Located $$ (Token (Op Complement  ) _)}
...
%right '->'
%left     'where'
%nonassoc 'then' 'else' 'when'
%nonassoc ':'
%left '\x2228'
%left '\x2227'
%nonassoc '==' '!=' '===' '!=='
%nonassoc '<' '>' '\x2264' '\x2265'
%left '^'
%right '#'
%left  '<<' '>>' '<<<' '>>>'
%left  '+' '-'
...
specBranches                     :: { [(Expr, Expr)] }
  : specBranch                        { [$1] }
  | specBranches '|' specBranch         { $3 : $1 }

specBranch                       :: { (Expr, Expr) }
  : expr 'when' expr                { ($3, $1) }
...
iexpr                            :: { Expr }
  ...
  | 'spec' specBranches 'else' iexpr { at ($1, $4) $ mkIf $2 $4 }
  ...
  | iexpr '\x2295'  iexpr                { binOp $1 (op ECXor         $2) $3 }
  | iexpr '\x2228' iexpr                { binOp $1 (op ECOr          $2) $3 }
  | iexpr '\x2227' iexpr                { binOp $1 (op ECAnd         $2) $3 }
  ...
  | '\x00AC' iexpr                       { unOp     (op ECCompl       $1) $2 }
...
aexpr                          :: { Expr                                   }
  ...
  | '(' '\x2295'    ')'                { at ($1,$3) $ ECon ECXor         }
  | '(' '\x2228'   ')'                { at ($1,$3) $ ECon ECOr          }
  | '(' '\x2227'   ')'                { at ($1,$3) $ ECon ECAnd         }
  ...
  | '(' '>'    ')'                { at ($1,$3) $ ECon ECGt          }
  | '(' '<'    ')'                { at ($1,$3) $ ECon ECLt          }
  | '(' '\x2264'   ')'                { at ($1,$3) $ ECon ECLtEq        }
  | '(' '\x2265'   ')'                { at ($1,$3) $ ECon ECGtEq        }
